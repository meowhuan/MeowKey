...
- the formal management bulk interface for inventory and posture reads
...
the manager can still read the formal management channel, but Debug-HID-only workflows such as CTAP bring-up, diagnostics, and destructive maintenance actions will be unavailable. That is expected behavior.

### Scope

The application is still a hybrid manager and maintenance workbench. It can now read the formal management channel, but CTAP exercise paths and destructive maintenance actions still lean on Debug HID. It is not a complete end-user manager yet.
...
- 正式管理 bulk 接口，用于 inventory / posture 只读视图
...
那么管理器仍然可以读取正式管理通道，但依赖 Debug HID 的 CTAP 联调、诊断和破坏性维护动作会不可用，这属于预期行为。

### 当前定位

这个应用目前仍然是“正式管理读路径 + Debug HID 维护路径”的混合工作台，还不是完整的普通终端用户管理器。
