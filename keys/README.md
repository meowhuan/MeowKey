...
- the key must be an `RSA` or `EC` private key in PEM form; OpenSSH private keys or `Ed25519` keys are not accepted by the release flow
...
- 私钥必须是 `RSA` 或 `EC` 的 PEM 格式；OpenSSH 私钥或 `Ed25519` 私钥不会被当前 release 流程接受
