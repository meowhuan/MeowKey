...
- the Windows WinUI manager shell can now perform per-credential delete and UP writes on the formal management channel, but the authorization model is still not `clientPIN` permission-token complete
...
- Windows WinUI 管理器壳层已经能在正式管理通道执行单条凭据删除和 UP 写入，但授权模型还没有达到 `clientPIN` permission-token 的完整语义
