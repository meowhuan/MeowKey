...
- the WebUI and Rust manager shell are still developer and maintenance tools
- the Windows WinUI manager shell exists now, but per-credential management and scoped authorization are still incomplete
...
- 开发构建和硬化构建会天然分叉
- WebUI 和 Rust 管理器壳层仍然主要是开发 / 维护工具
- Windows WinUI 管理器壳层已经存在，但单条凭据管理和权限范围授权仍未补齐
