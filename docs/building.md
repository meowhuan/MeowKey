...
- the browser UI depends on Debug HID; the Rust shell can use the formal management channel for reads plus manager-safe writes (per-credential delete and user-presence writes) with local-UP-gated short-lived authorization
...
- 浏览器调试台依赖 Debug HID；Rust 壳层即使没有它也能走正式管理通道读写（单条凭据删除与 user presence 写入，带本地在场确认和短时授权）
